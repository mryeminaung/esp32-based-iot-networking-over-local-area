...
export default function CreateUserModal({ open, onClose, onCreated }: CreateUserModalProps) {
 ...
 Password: <span className="font-mono text-text-secondary">{password}</span>
 </p>
 )}
 </div>
 ...
 variant="outline"
 onClick={handleClose}
 className="flex-1 flex items-center justify-center gap-2 px-4 rounded-lg border border-border text-text-secondary text-sm font-medium hover:bg-gray-50 transition-colors cursor-pointer">
 Cancel
 </Button>
 <Button
 type="submit"
 disabled={submitting}
 className="flex-1 flex items-center justify-center gap-2 px-4 rounded-lg bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white text-sm font-medium transition-colors cursor-pointer disabled:cursor-not-allowed">
 {submitting ? (
 <>
