...
function getCondition(value: number) {
	if (value <= 30) return { label: "DRY", color: "#ef4444", bgClass: "bg-danger/10", textClass: "text-danger" };
	if (value < 50) return { label: "MOIST", color: "#f59e0b", bgClass: "bg-warning/10", textClass: "text-warning" };
	return { label: "WET / OPTIMAL", color: "#10b981", bgClass: "bg-success/10", textClass: "text-success" };
}
...
export default function RadialGauge({ value, size = 200 }: RadialGaugeProps) {
	...
	const condition = getCondition(clamped);
